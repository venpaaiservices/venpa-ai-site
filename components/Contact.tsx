...
<<<<<<< HEAD
    ...
    if (formData.date) {
      ...
      const selectedDate = formData.date; // YYYY-MM-DD
      
      if (selectedDate < todayString) {
        newErrors.date = 'Cannot schedule a past date';
      } else if (selectedDate === todayString && formData.time) {
        const [selHours, selMinutes] = formData.time.split(':').map(Number);
        const selectedTime = selHours * 60 + selMinutes;
        
        if (selectedTime < currentMinutes + 1) { // Adding 1 minute buffer
          newErrors.time = 'Cannot schedule a past time';
        }
      }
    }
...
<<<<<<< HEAD
  const getISTDetails = () => {
    ...
    return { dateString, currentMinutes: h * 60 + m };
  };
...
>>>>>>> a004ee3e75b3ed3fe07ff19cab6ed50b5c25da6c
  ...
  return (
    <section id="contact" className="scroll-mt-16 bg-slate-50/50 dark:bg-slate-950/30 backdrop-blur-sm pt-24 pb-12 relative overflow-hidden">
       {/* Decorative elements */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 mb-12">
          ...
          <motion.div 
            ...
            viewport={{ once: true }}
            transition={{ duration: 0.8 }}
            className="bg-white dark:bg-slate-900 rounded-3xl p-8 md:p-10 shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden"
          >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <input 
                      ...
                      value={formData.date}
                      onChange={handleChange}
                      className={`w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border ${errors.date ? 'border-red-500' : 'border-slate-200 dark:border-slate-800'} focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all dark:text-white`}
                    />
                    {errors.date && <p className="text-xs text-red-500 mt-1">{errors.date}</p>}
                  </div>
                  <div className="space-y-2">
                    <label htmlFor="time" className="text-sm font-semibold text-slate-700 dark:text-slate-300">Preferred Time (IST)</label>
                    <input 
                      type="time" 
                      id="time" 
                      value={formData.time}
                      onChange={handleChange}
                      className={`w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-950 border ${errors.time ? 'border-red-500' : 'border-slate-200 dark:border-slate-800'} focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition-all dark:text-white`}
                    />
...
>>>>>>> a004ee3e75b3ed3fe07ff19cab6ed50b5c25da6c
                ...
                >
                  {formState === 'submitting' ? (
                    ...
                    <>
                      Book Your Technical Discovery <Send size={18} />
                    </>
                  )}
                </button>
              </form>
